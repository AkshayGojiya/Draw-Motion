...
toolElementCollection.forEach((tool) => {
  let topval =
    tool.getBoundingClientRect().top - canvas2.getBoundingClientRect().top;
  let bottomval =
    tool.getBoundingClientRect().bottom - canvas2.getBoundingClientRect().top;
  let rightval =
    tool.getBoundingClientRect().right - canvas2.getBoundingClientRect().left;
  let leftval =
    tool.getBoundingClientRect().left - canvas2.getBoundingClientRect().left;

  toolCoordinatesCollection.push({
    top: topval,
    bottom: bottomval,
    right: rightval,
    left: leftval,
...
const brushSize = new Tools("brushSize", toolCoordinatesCollection[1], {
  size: 5,
});
...
let drawfunc = function () {
  ...
  if (
    ...
    calcDistance(previousCoordinates, [cursor.x, cursor.y]) < 100
  ) {
...
const brush = new Tools("brush", toolCoordinatesCollection[0], {
  method: drawfunc,
});
const erasor = new Tools("erasor", toolCoordinatesCollection[2], {
  method: erasefunc,
});
const color = new Tools("color", toolCoordinatesCollection[6], {
  color: "black",
});
const shape = new Tools("shape", toolCoordinatesCollection[7], {
  shape: "",
  left: null,
  top: null,
  x: null,
...
const redo = new Tools("redo", toolCoordinatesCollection[3], {
  method: redofunc,
  isActive: false,
});
const undo = new Tools("undo", toolCoordinatesCollection[4], {
  method: undofunc,
  isActive: false,
});
const clearCanvas = new Tools("clearCanvas", toolCoordinatesCollection[5], {
  method: clearCanvasfunc,
});
...
let sizeBarCoordinates = {
  top:
    inputRange.getBoundingClientRect().top -
    canvas2.getBoundingClientRect().top,
  bottom:
    inputRange.getBoundingClientRect().bottom -
    canvas2.getBoundingClientRect().top,
  left:
    inputRange.getBoundingClientRect().left -
    canvas2.getBoundingClientRect().left,
  right:
    inputRange.getBoundingClientRect().right -
    canvas2.getBoundingClientRect().left,
};
...
for (let element of colorContainer.children) {
  colorValuesCoordinates.push({
    top:
      element.getBoundingClientRect().top - canvas2.getBoundingClientRect().top,
    bottom:
...
function saveDrawingState() {
  if (
    cursor.currentTool.name == "erasor" ||
    cursor.currentTool.name == "brush"
  ) {
...
function setRangeValue() {
  ...
  if (
    cursor.x > sizeBarCoordinates.left &&
    cursor.x < sizeBarCoordinates.right &&
    cursor.y > sizeBarCoordinates.top - 10 &&
    cursor.y < sizeBarCoordinates.bottom + 10
  ) {
  ...
  if (shape.action.x > shape.action.left && shape.action.y > shape.action.top) {
    if (cursor.currentTool.action.shape == "rectangle") {
      ...
      ctx2.strokeRect(
        shape.action.left,
        shape.action.top,
        shape.action.x - shape.action.left,
        shape.action.y - shape.action.top
      );
    } else if (cursor.currentTool.action.shape == "circle") {
      ctx2.beginPath();
      ctx2.strokeStyle = color.action.color;
  ...
  fetch("http://localhost:3000/coordinates")
    ...
    .then((data) => {
      ...
      if (isHover && cursor.mode == "select") {
        ...
        if (
          cursor.currentTool.name == "clearCanvas" ||
          cursor.currentTool.name == "redo" ||
          cursor.currentTool.name == "undo"
        ) {
